...
namespace Awrosoft.Utilities.HtmlToQuestPDF.Components
{
    internal class ParagraphComponent : IComponent
    {
        ...
        {
            if (!element.IsSpan()) return (span) => span.Style(GetTextStyle(element));
            
            var style = element.GetTextStyleFromSpan();
